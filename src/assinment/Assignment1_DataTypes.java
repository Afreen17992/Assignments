package assinment;

public class Assignment1_DataTypes {

	public static void main(String[] args) {


		// 1. Temperature of a city in degrees Celsius: 25.5
		float Temperature = 25.5f;
		System.out.println("1. Temperature of City in Degree Celsius : "+ Temperature);
		
		// 2. Whether a customer has placed an order: true or false
		boolean CustomerOrder = true;
		if (CustomerOrder) {
		    System.out.println("2. The customer has placed an order.");
		} else {
		    System.out.println("2. The customer has not placed an order.");
		}

		// 3. Person's phone number: "123-456-7890"
		String PhoneNumber ="123-456-7890";
		System.out.println("3. Phone Number : " + PhoneNumber);
		
		// 4. Amount of money in a customer's bank account: 1000.50
		float Money = 1000.50f;
		System.out.println("4. Amount of Money in Customer's Bank Account : "+ Money);
		
		// 5. Person's email address: "john.doe@example.com"
		String EmailAddress = "afreen17992@gmail.com";
		System.out.println("5. Person's Email Address : " + EmailAddress);
		
		// 6. Coordinates of a location (latitude, longitude): 37.7749, -122.4194
		double latitude = 37.7749 , longitude = -122.4194;
		System.out.println("6. Coordinates of a location (latitude, longitude): " + latitude + "," + longitude );
	
		
		// 7. Person's marital status: true or false
		boolean MaritalStatus = true ;
		if (MaritalStatus) {
		    System.out.println("7. Person is married.");
		} else {
		    System.out.println("7. Person is not married.");
		}
		
		// 8. Person's occupation: "Software Engineer"
		String occupation = "Software Engineer";
		System.out.println("8. Person's occupation : " + occupation);
		
		// 9. Person's favourite colour: "Blue"
		String FavouriteColour = "Blue";
		System.out.println("9. Person's favourite colour : " + FavouriteColour);
		
		// 10.Current year: 2023
		int CurrentYear = 2023;
		System.out.println("10. Current year : "+ CurrentYear);
		
		// 11. Number of followers on a social media platform: 1,000,000
		int Followers = 1_000_000;
		System.out.println("11. Number of followers on a social media platform : " + Followers);
		
		// 12.Rating of a movie: 7.5
		float MovieRating = 7.5f;
		System.out.println("12. Rating of a movie : "+ MovieRating);
		
		// 13.Person's blood type: 'A'
		char BloodType = 'A';
		System.out.println("13. Person's blood type : "+ BloodType);
		
		// 14.Title of a book: "To Kill a Mockingbird"
		String BookTitle = "To Kill a Mockingbird";
		System.out.println("14. Title of a book : "+ BookTitle);
		
		// 15. Number of employees in a company: 500
		int EmployeesNumbers = 500;
		System.out.println("15. Number of employees in a company : "+ EmployeesNumbers);
		
		// 16. Time of an event: 2:30 PM
		String EventTime = "2:30 PM";
		System.out.println("16. Time of an event : " + EventTime);
		
		// 17. Name of a country: "United States"
		String CountryName = "United States";
		System.out.println("17. Name of a country : " + CountryName);
		
		// 18. Person's eye color: "Brown"
		String EyeColor = "Brown";
		System.out.println("18. Person's eye color : "+ EyeColor);
		
		// 19. Person's Birth Place: "New York City"
		String BirthPlace = "United States";
		System.out.println("19. Person's Birth Place : " + BirthPlace);
		
		// 20. Distance between two cities: 200.5
		float Distance = 200.5f;
		System.out.println("20. Distance between two cities : "+ Distance);

		
	}

}
